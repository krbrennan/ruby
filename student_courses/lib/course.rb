...
class Course
  attr_accessor :students, :name, :department, :credits, :days, :time_block
  ...
  def conflicts_with?(other_course)
    common_days = days.select{|day| other_course.days.include?(day)}
    common_days.each{|day| return true if time_block == other_course.time_block}
    false
  end
