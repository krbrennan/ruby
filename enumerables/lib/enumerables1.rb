...
def price_is_right(bids, actual_retail_price)
  closest = bids.first
  bids.select{|bid| (actual_retail_price - bid ) > 0}.max
end
...
def num_factors(number)
  total = 0
  (1..number).each{|num| total += 1 if number % num == 0}
  total
end
...
def ordered_vowel_word?(word)
  vowel = %w{a e i o u}
  vowels = word.chars.select{|letter| vowel.include?(letter)}
  vowels == vowels.sort
end
...
def products_except_me(numbers)
  tot = array_product(numbers)
  numbers.map{|ele| tot / ele}
end
